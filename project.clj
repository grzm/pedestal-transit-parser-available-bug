(defproject trapar "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "trapar.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.1"]]}
             :uberjar {:aot [trapar.server]}}
  :main ^{:skip-aot true} trapar.server)
